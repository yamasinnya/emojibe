class BootSceneMulti extends Phaser.Scene {
  constructor() { super({ key: 'BootSceneMulti' }); }

  create() {
    const W = this.scale.width;
    const H = this.scale.height;
    this.add.graphics().fillStyle(0x2a1a0a, 1).fillRect(0, 0, W, H);
    this.add.text(W / 2, H / 2 - 20, 'えもじべ 対人戦', {
      fontSize: '28px', color: '#f5e6c8',
    ...
    const SIZE = 72;
    EMOJIS.forEach(e => this._makeEmojiTexture(e.emoji, SIZE));
    ['🎲', '🌸', '⭐', '🎯', '🏆', '⏱️', '🆚'].forEach(e => this._makeEmojiTexture(e, SIZE));

    this.time.delayedCall(400, () => this.scene.start('JoinScene'));
  }
